...
func _on_visible_changed() -> void:
	#clear the list, then update it. Repopulate the list fresh every time you open the menu
	...
	if visible == true:
		...
		for q in QuestManager.current_quests:
			...
			new_q_item.focus_entered.connect( _update_quest_details.bind( new_q_item.quest ) )
	
	pass

func _update_quest_details( q : Quest ) -> void:
