...
const COLORS = [ Color(1,0,0), Color(1,1,0), Color(1,1,0), Color(0,1,1), Color(0,0,1),
Color(1,0,1) ]
...
func _ready() -> void:
	...
	if Engine.is_editor_hint():
		child_entered_tree.connect( gather_patrol_locations ) #when new child is added in the editor
		child_order_changed.connect( gather_patrol_locations ) #when child order changes in the editor
		return
	super() #call the ready functionality in the parent as well
...
func gather_patrol_locations( _n : Node = null ) -> void:
	patrol_locations = [] #reset the array to empty before filling again
