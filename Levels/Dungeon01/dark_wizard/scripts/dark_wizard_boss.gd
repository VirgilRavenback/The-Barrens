...
@onready var hit_box: HitBox = $BossNode/hit_box
...
func _process(_delta: float) -> void:
	...
	hand_02_up.frame = hand_01.frame + 4
	hand_01_side.position = hand_01.position
	hand_01_side.frame = hand_01.frame +8
	hand_02_side.position = hand_01.position
	hand_02_side.frame = hand_01.frame + 12
	pass
...
func teleport( _location : int ) -> void:
	...
	#choose a new location and move there
	boss_node.global_position = positions[ _location ]
	current_position_index = _location
...
func explosion( _position : Vector2 = Vector2.ZERO ) -> void:
	...
	e.global_position = boss_node.global_position + _position
