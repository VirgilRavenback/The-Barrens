	...
	for t in get_tree().get_nodes_in_group( "tilemaps" ):
		if t is TileMapLayer:
			if t.tile_set.get_custom_data_layer_by_name( "footstep_type" ) == -1:
				continue
			#vector2i is a vector that only uses integers
			#this var gets the global position of the node, converts it to a local position
			#and then finds the corresponding cell on the tile map layer below it.
